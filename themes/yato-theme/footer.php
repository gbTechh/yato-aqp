<footer class="bg-bg-primary text-white relative footer-clip-path pt-32 overflow-hidden" id="site-footer">
                ...
                <div>
                    ...
                    <div class="flex gap-4">
                        ...
                        <?php if (get_option('yato_social_tiktok')): ?>
                            <a href="<?php echo esc_url(get_option('yato_social_tiktok')); ?>" target="_blank"
                                class="bg-bg-primarydark hover:bg-yellow-500 text-white p-3 rounded-full transition-all duration-300 transform hover:-translate-y-1">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                        ...
                        <?php if (get_option('yato_social_youtube')): ?>
                            <a href="<?php echo esc_url(get_option('yato_social_youtube')); ?>" target="_blank"
                                class="bg-bg-primarydark hover:bg-yellow-500 text-white p-3 rounded-full transition-all duration-300 transform hover:-translate-y-1">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M21.58 5.4a2.76 2.76 0 0 0-1.94-1.95C17.92 3 12 3 12 3s-5.92 0-7.64.45A2.76 2.76 0 0 0 2.42 5.4C2 7.12 2 12 2 12s0 4.88.42 6.6a2.76 2.76 0 0 0 1.94 1.95C6.08 21 12 21 12 21s5.92 0 7.64-.45a2.8 2.8 0 0 0 1.94-1.95C22 16.88 22 12 22 12s0-4.88-.42-6.6zM9.75 15.02V8.98L15 12l-5.25 3.02z" />
                                </svg>
                            </a>
                        <?php endif; ?>

                        <?php if (get_option('yato_social_twitter')): ?>
                            <a href="<?php echo esc_url(get_option('yato_social_twitter')); ?>" target="_blank"
                                class="bg-bg-primarydark hover:bg-yellow-500 text-white p-3 rounded-full transition-all duration-300 transform hover:-translate-y-1">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                        ...
                        <?php if (get_option('yato_social_linkedin')): ?>
                            <a href="<?php echo esc_url(get_option('yato_social_linkedin')); ?>" target="_blank"
                                class="bg-bg-primarydark hover:bg-yellow-500 text-white p-3 rounded-full transition-all duration-300 transform hover:-translate-y-1">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                ...
                <div class="text-center md:text-right">
                    <div class="flex flex-wrap justify-center md:justify-end gap-4 text-sm">
                        ...
                        wp_nav_menu(array(
                            ...
                            'link_class' => 'text-white font-medium hover:text-yellow-300 transition-colors'
                        ));
                        ?>
                    </div>
...
<?php if (get_option('yato_contact_whatsapp')): ?>
<?php
    $wa_phone   = preg_replace('/[^0-9]/', '', get_option('yato_contact_whatsapp'));
    $wa_message = get_option('yato_whatsapp_message', '¡Hola! Me gustaría obtener más información.');
...
<a href="<?php echo esc_url($wa_url); ?>" target="_blank" rel="noopener noreferrer"
    ...
    class="fixed bottom-6 right-6 z-50 flex items-center justify-center whatsapp bg-green-500 hover:bg-green-600 text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 group js-whatsapp-float">
    <svg class="w-9 h-9" fill="currentColor" viewBox="0 0 24 24">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.76.982.998-3.677-.236-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.9 6.994c-.004 5.45-4.438 9.88-9.888 9.88m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.333.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.333 11.893-11.893 0-3.18-1.24-6.162-3.495-8.411" />
    </svg>
    <span class="absolute right-16 bg-gray-900 text-white text-xs font-medium px-3 py-1.5 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none shadow-md js-whatsapp-label">¡Escríbenos!</span>
</a>
<?php endif; ?>
